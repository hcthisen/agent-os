#!/bin/sh
# Test the core MCP tools against live Supabase.
...
# Get a builder agent ID for testing.
...
# 5b. Also insert a memory chunk for search.
...
# 9. context_refresh - call build_context_pack
echo "--- 9. context_refresh ---"
CONTEXT=$($DB_CMD -c "SELECT jsonb_typeof(build_context_pack('$TASK_ID'));")
echo "Context pack type: $CONTEXT"

# Verify the task state machine by completing the task.
echo ""
