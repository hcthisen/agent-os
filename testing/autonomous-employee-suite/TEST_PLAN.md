...
| `04` | Prospect demo build | gated | `fixing` | `8` | `yes` | `no` | `no` | `Relay still only surfaced the stale root checklist after the resumed run; it never forwarded the newer builder blocked_on_human handoff that contained the true end-of-run publish blocker.` | Patch blocked-update candidate selection so relay prefers the freshest actionable blocker in the request tree, redeploy, reset, and rerun the resumed path. |
